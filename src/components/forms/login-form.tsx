...
// `callbackUrl` arrives from a query string and is attacker-controllable.
// NextAuth validates same-origin in `result.url`; the local fallback used
// when `result.url` is unset must also stay same-origin to prevent open
// redirects (e.g. `?callbackUrl=//evil.com`).
...
export function LoginForm({ callbackUrl }: LoginFormProps) {
  ...
  async function handleCredentialsSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (result.error) {
        // Auth.js v5 surfaces our custom CredentialsSignin.code in result.code
        // (and in the URL as ?error=...). Treat EmailNotVerified specially so
        // we can offer to resend the link instead of a generic error.
        const code = (result as { code?: string }).code;
        if (code === "EmailNotVerified") {
          setFormError({
            message: "Your email is not verified yet. Check your inbox or request a new link.",
      ...
      const safeFallback = safeInternalPath(callbackUrl, "/dashboard");
      window.location.assign(result.url ?? safeFallback);
    } finally {
      setSubmitting(false);
    }
  }
    ...
    <div className="space-y-6">
      ...
      <div className="relative">
        ...
        <div className="relative flex justify-center text-xs uppercase tracking-wider">
          <span className="bg-slate-50 px-3 text-slate-400 lg:bg-slate-50">or sign in with email</span>
        </div>
      </div>
        ...
        {formError ? (
          ...
          >
            ...
            {formError.unverifiedEmail ? (
              <Link
                href={`/verify-email/pending?email=${encodeURIComponent(formError.unverifiedEmail)}`}
                className="ml-6 inline-block font-medium text-red-800 underline"
              >
