...
export function GroupTabs({ groupId }: GroupTabsProps) {
  ...
  return (
    ...
    >
      <div className="inline-flex min-w-full gap-1 border-b border-white/[0.06]">
        {tabs.map((tab) => {
          const isActive = tab.key === active;
          const className = cn(
            "whitespace-nowrap border-b-2 px-4 py-2.5 text-sm font-medium transition focus:outline-none focus-visible:ring-2 focus-visible:ring-accent focus-visible:ring-offset-2 focus-visible:ring-offset-bg",
            isActive
              ? "border-accent text-accent"
              : "border-transparent text-text-secondary hover:border-white/10 hover:text-text-primary",
          );
